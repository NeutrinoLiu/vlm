...
scene_list = read_txt_list("/mnt/bn/nlhei-nas/liubangya/proj/vlm/datasets/scannetpp/preprocess/sc_name_all_valid.txt")
...
def process_single_scene(sc):
    ...
    visibility = get_visiblity_from_cache(scene = ScannetppScene_Release(sc, data_root=SCANNETPP_ROOT),
                                          ...
                                          image_type="dslr",
                                          subsample_factor=1,)
    expected_cache_path = os.path.join(RASTER_CACHE, f"{sc}.pth")
    ...
    for frame in tqdm(frame_list, desc=f"Processing scene {sc}"):
        ...
        corresponding_visibility_cache = os.path.join(RASTER_RES, sc, f"{frame}.pth")
        if not os.path.exists(corresponding_visibility_cache):
            continue
