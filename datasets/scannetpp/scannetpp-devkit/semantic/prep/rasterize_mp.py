...
def per_proc(triplet):
    ...
    cfg, scene_id, device = triplet
    a = torch.Tensor([1])
    a.to(device)
    print(f"working on {scene_id}, in gpu {device}")
    ...
    for batch_ndx, batch_start_ndx in enumerate(tqdm(batch_start_indices, desc='batch', leave=False)):
        ...
        if cfg.skip_existing:
            ...
            mask = torch.tensor([not p.exists() for p in rasterout_paths_orig], dtype=torch.bool)
            batch_poses = batch_poses[mask]
...
def main(cfg : DictConfig) -> None:
    ...
    input_data = []
    for i, sid in enumerate(scene_list):
        device = f"cuda:{i % num_cuda}"
        input_data.append(
            (cfg, sid, device)
        )

    njob = cfg.get("job", num_cuda * 2)
    with Pool(processes=njob) as pool:
        for result in tqdm(pool.imap(per_proc, input_data)):
            continue
